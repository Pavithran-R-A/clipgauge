...
export default function App() {
  ...
  const startRun = useCallback(
    async (source: string, provider: string, captions: string, model?: string, endpoint?: string) => {
      setRunning(false)
      setCancelling(false)
      setRunError(null)
      setRunNotice(null)
      ...
      setActiveJob(null)
      try {
        const preflight = await api.preflight(provider, model, endpoint)
        ...
        await api.runJob(source, provider, captions, model, endpoint)
      } catch (error) {
        setRunning(false)
