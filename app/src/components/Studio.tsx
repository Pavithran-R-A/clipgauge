...
export default function Studio({ running, cancelling, startedAt, stages, error, notice, onRun, onCancel, onNavigate, selectedProvider, onSelectProvider }: Props) {
    ...
    <div className="workspace-page create-page">
      ...
      <div className="create-layout">
        <div className="create-main-column">
          ...
          <div className="create-action-row"><button type="button" className="button button-primary create-button" onClick={() => source.trim() && onRun(source.trim(), provider === 'other' ? 'clipgauge-local' : provider, captions)} disabled={running || !source.trim()}><Play size={17} fill="currentColor" aria-hidden="true" />{running ? 'Creating clips…' : 'Create clips'}</button>{running && <button type="button" className="button button-quiet" onClick={onCancel} disabled={cancelling}>{cancelling ? 'Cancelling…' : 'Cancel'}</button>}<span className="action-note"><LockKeyhole size={14} aria-hidden="true" /> No account required for local scoring</span></div>
        </div>
      ...
      {hasProgress && <section className="processing-panel card-surface" aria-live="polite"><div className="processing-header"><div><p className="section-eyebrow">Creating your clips</p><h2>{running ? 'Finding the moments worth sharing.' : 'Your video is ready to review.'}</h2></div><span className="elapsed-pill">{formatElapsed(elapsed)} elapsed</span></div><div className="timeline">{STAGE_ORDER.map((stage) => { const current = stages[stage]; const done = Boolean(current && current.fraction >= 1); const active = Boolean(current && !done) || (!current && running && stage === STAGE_ORDER.find((item) => !stages[item])); return <div className={`timeline-step ${done ? 'is-done' : ''} ${active ? 'is-active' : ''}`} key={stage}><span className="timeline-dot" aria-hidden="true">{done ? '✓' : active ? '•' : ''}</span><span>{current?.displayStage ?? STAGE_LABELS[stage]}</span>{active && current?.operation && <small>{current.operation}</small>}</div> })}</div>{notice && <p className="inline-message" role="status">{notice}</p>}{error && <p className="error-message" role="alert">{error}</p>}<details className="technical-disclosure"><summary>Show technical details</summary><div className="technical-progress-list">{Object.entries(stages).map(([name, stage]) => <div key={name}><span>{name}</span><span>{stage.message}</span></div>)}</div></details></section>}
    </div>
  )
}
