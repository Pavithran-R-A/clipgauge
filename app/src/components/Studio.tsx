    ...
    <div className="workspace-page create-page">
      ...
      <div className="create-layout">
        <div className="create-main-column">
          ...
          <section className="choice-section" aria-labelledby="quality-heading"><div className="section-heading"><div><p className="section-eyebrow">Scoring preference</p><h2 id="quality-heading">Choose scoring mode</h2><p className="section-caption">Cloud scoring never starts without your explicit mode choice.</p></div></div><div className="choice-card-grid">{QUALITY_OPTIONS.map((option) => { const disabled = option.id === 'private' ? readiness.locality === 'cloud' : readiness.locality === 'local' || !cloudAvailable; return <button type="button" key={option.id} className={`choice-card choice-neutral ${qualityMode === option.id ? 'is-selected' : ''}`} onClick={() => { if (!disabled) setQualityMode(option.id) }} aria-pressed={qualityMode === option.id} disabled={disabled} aria-disabled={disabled}><span className="choice-card-icon"><LockKeyhole size={17} aria-hidden="true" /></span><span><strong>{option.name}</strong><small>{option.description}</small></span>{qualityMode === option.id && <span className="choice-selected">Selected</span>}</button> })}</div>{qualityMode === 'private' && readiness.locality === 'local' && <p className="field-help" role="status">{providerName(provider)} scores here using {selectedModelLabel}. No cloud provider receives candidate data.</p>}{qualityMode === 'private' && readiness.locality === 'cloud' && <p className="field-help" role="alert">Private mode requires a local provider. Choose ClipGauge Local, Ollama, or LM Studio.</p>}{!cloudAvailable && readiness.locality === 'local' && <p className="field-help" role="note">Configure a cloud provider and model in AI Providers before choosing Hybrid or Best Quality.</p>}{qualityMode !== 'private' && !cloudAvailable && readiness.locality === 'cloud' && <p className="field-help" role="note">{readiness.blocking_reasons.join('. ') || 'Configure a capable cloud provider and model'} before choosing {qualityMode === 'balanced' ? 'Balanced' : 'Best Quality'}.</p>}{qualityMode !== 'private' && !modeBlocked && <p className="field-help" role="note">What leaves this computer: candidate transcript, candidate metadata, and sampled images when visual scoring is supported. The full source file stays on this computer.</p>}</section>
