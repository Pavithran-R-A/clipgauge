...
export default function SupportPage({ onBack, onNavigate, provider = 'clipgauge-local', currentJobId, currentDiagnosticId }: Props) {
  ...
  async function makeBundle() {
    ...
    try { await api.generateSupportBundle(currentJobId ?? undefined, currentDiagnosticId ?? undefined); setMessage('Support bundle created. Review it before sharing.') } catch (error) { setMessage(friendlyErrorMessage(error, 'The support bundle could not be created. Retry the action.')) }
  }
