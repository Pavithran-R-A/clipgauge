...
import { api } from '../api'
import type { GpuDiagnostics, LocalSetupInventory, ManagedAssetRow, SetupProgressEvent, YouTubeReadiness } from '../types'
...
type GpuUiState = 'CHECKING' | 'READY' | 'CPU_ONLY' | 'DEGRADED' | 'REPAIR_REQUIRED' | 'PROBE_FAILED'

function gpuUiState(load: SetupLoadState<GpuDiagnostics>, diagnostics: GpuDiagnostics | null): { state: GpuUiState; reason: string } {
  if (load.phase === 'error') return { state: 'PROBE_FAILED', reason: load.message ?? 'The GPU probe failed. Retry diagnostics.' }
...
export default function SetupCenter({ onBack, onUseLocal }: Props) {
  ...
  const [gpuLoad, setGpuLoad] = useState<SetupLoadState<GpuDiagnostics>>({ phase: 'loading' })
  ...
  const gpuStatus = gpuUiState(gpuLoad, gpuDiagnostics)
  return (
    <div className="page-frame setup-page">
      ...
      <section className="card-surface gpu-diagnostics" aria-live="polite"><div className="section-heading"><div><p className="section-eyebrow">Speech acceleration</p><h2>GPU diagnostics</h2><p className="section-caption">ClipGauge checks transcription and word alignment separately.</p></div><span className={`status-pill tone-${gpuStatus.state === 'READY' ? 'ready' : gpuStatus.state === 'CPU_ONLY' ? 'neutral' : 'warning'}`}><span className="status-dot" aria-hidden="true" />{gpuStatus.state}</span></div><p className="inline-message" role={gpuStatus.state === 'PROBE_FAILED' ? 'alert' : 'status'}>{gpuStatus.reason}</p>{gpuStatus.state === 'PROBE_FAILED' && <button type="button" className="button button-secondary" onClick={() => void refreshGpu()} disabled={gpuLoad.phase === 'loading' || gpuBusy}>Retry GPU diagnostics</button>}{gpuDiagnostics && <div className="technical-table"><div className="technical-row"><span><strong>GPU</strong><small>{gpuDiagnostics.hardware?.nvidia?.gpus?.[0]?.name ?? 'Not detected'}</small></span><span>{gpuDiagnostics.hardware?.nvidia?.gpus?.[0]?.driver ? `Driver ${gpuDiagnostics.hardware.nvidia.gpus[0].driver}` : 'Unavailable'}</span><span>{gpuDiagnostics.hardware?.nvidia?.verified ? 'Verified' : 'Unavailable'}</span></div><div className="technical-row"><span><strong>Transcription</strong><small>CTranslate2 CUDA</small></span><span>{gpuDiagnostics.hardware?.cuda_ctranslate2?.compute_types?.join(', ') ?? 'Unavailable'}</span><span>{gpuDiagnostics.hardware?.cuda_ctranslate2?.verified ? 'Ready' : 'CPU fallback'}</span></div><div className="technical-row"><span><strong>Word alignment</strong><small>PyTorch / WhisperX</small></span><span>{gpuDiagnostics.hardware?.pytorch_cuda?.compiled_cuda ? `CUDA ${gpuDiagnostics.hardware.pytorch_cuda.compiled_cuda}` : 'CPU build'}</span><span>{gpuDiagnostics.hardware?.pytorch_cuda?.verified ? 'GPU ready' : 'CPU alignment'}</span></div><div className="technical-row"><span><strong>Managed libraries</strong><small>Approved CUDA and cuDNN files</small></span><span>{gpuDiagnostics.cuda_runtime_ready && gpuDiagnostics.cudnn_runtime_ready ? 'Verified' : 'Repair needed'}</span><span>{gpuDiagnostics.environment?.state ?? 'Unknown'}</span></div></div>}<div className="detail-actions"><button type="button" className="button button-secondary" onClick={repairGpu} disabled={gpuBusy}>{gpuBusy ? 'Repairing GPU acceleration…' : 'Repair GPU acceleration'}</button><button type="button" className="button button-quiet" onClick={() => void refreshGpu()} disabled={gpuBusy}>Refresh diagnostics</button></div></section>
