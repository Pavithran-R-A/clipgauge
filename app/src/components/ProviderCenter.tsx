    ...
    <div className="page-frame providers-page">
      ...
      <div className="providers-layout">
        ...
        <aside className="provider-detail" aria-labelledby="provider-detail-title">
          ...
          <div className="detail-actions">{active.id === 'clipgauge-local' && !localReady ? <button type="button" className="button button-primary" onClick={() => onOpenSetup?.() ?? onBack()}>Set up local AI<ChevronRight size={16} aria-hidden="true" /></button> : <button type="button" className="button button-primary" onClick={testConnection} disabled={testing}>{testing ? 'Testing…' : 'Test connection'}<ChevronRight size={16} aria-hidden="true" /></button>}<button type="button" className="button button-secondary" onClick={() => { onSelectProvider(active.id); onBack() }}>Use for next clip</button></div>
