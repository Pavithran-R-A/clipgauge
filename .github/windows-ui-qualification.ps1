...
function Validate-NativeDialogEvidence {
  ...
  if ($metadata.content_text_read_method -notin @('winapp-ui-get-value-json', 'direct-uia-textpattern-json') -or -not $metadata.content_text_nonempty -or -not $metadata.expected_provider_observed -or -not $metadata.non_revocation_phrase_observed -or -not $metadata.ok_control_observed -or -not $metadata.cancel_control_observed) { throw "native dialog machine-readable UIA evidence incomplete: $metadataPath" }
