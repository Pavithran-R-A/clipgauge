...
test('uses a deterministic multi-segment controlled speech acceptance input', () => {
  const start = workflow.indexOf('  model-e2e-release:')
  const end = workflow.indexOf('  release-metadata:', start)
  const modelJob = workflow.slice(start, end)
  assert.match(modelJob, /anullsrc=r=16000:cl=mono:d=1/)
  assert.match(modelJob, /flite=text=/)
  assert.match(modelJob, /payment launch fail/)
  assert.match(modelJob, /expired security certificate/)
  assert.match(modelJob, /every order succeeded/)
  assert.match(modelJob, /concat=n=5:v=0:a=1/)
  assert.match(modelJob, /asplit=2\[silence1\]\[silence2\]/)
  assert.match(modelJob, /\[story1\]\[silence1\]\[story2\]\[silence2\]\[story3\]concat/)
  assert.match(modelJob, /acceptance\.flac/)
  assert.doesNotMatch(modelJob, /v041-jfk\.flac/)
  assert.doesNotMatch(modelJob, /laughing-esc50\.wav/)
