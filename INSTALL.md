...
Do not treat an unsigned artifact as proof of publisher identity. Verify `SHA256SUMS`, review package metadata, and understand that Windows may show SmartScreen warnings. macOS builds that are not signed and notarized may be blocked or warned about by Gatekeeper. Distribution, signing, notarization, and trust decisions remain explicit release limitations until publication evidence exists.

## Windows and macOS release artifacts

The v0.6.1 release workflow built the public Windows x64 NSIS installer from the v0.6.1 tag after native tests, resource staging, silent installation, and installed-process smoke checks passed. The v0.6.2 patch must repeat those checks before publication. Installers are unsigned. Do not describe them as Authenticode-signed.
